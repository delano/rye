...
Now supports JRuby 1.3+!

== Overview

Rye is similar to Rush[http://rush.heroku.com] but everything happens over SSH (no HTTP daemon) and the default settings err on the side of safety. For example, file globs and the "rm" command are disabled so unless otherwise specified, you can't do this: <tt>rbox.rm('-rf', '/etc/**/*')</tt>.
...
* <tt>$ sudo gem install rye</tt>
* <tt>$ sudo gem install delano-rye --source http://gems.github.com/</tt>
* <tt>$ git clone git://github.com/delano/rye.git</tt>
